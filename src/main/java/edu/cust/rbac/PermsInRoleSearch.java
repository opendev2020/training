...
public class PermsInRoleSearch extends Search {
	...
	protected String getTables(ListTemplate lt) {
		...
		return "(select a.* from c_perm a left outer join (select c_perm_id from c_role_perm where c_role_id=?) b on a.c_id=b.c_perm_id where b.c_perm_id is not null) c";
	}
