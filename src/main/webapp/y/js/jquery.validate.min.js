...
var showErrorsInAlert = function(errors) {
    var statusMessage = "";
    var i = 1;
    for (var name in errors) {
        statusMessage += "<h3>" + i + ". " + errors[name] + '</h3>';
        i++;
    }
    if (statusMessage != "") {
        $.showDialog({ id: "statusMessage", title: "未通过验证", modal: true,
            content: statusMessage, buttons: { "确定": function() { $(this).dialog('close'); } }, resizable: false
        });
    }
}